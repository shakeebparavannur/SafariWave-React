...
const FeturedPackages = () => {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_APP_URL}/api/Packages/feutures`).then(
      (response) => setFeaturedPack(response.data),
      setTimeout(() => {
        setLoading(false);
      }, 3000)
    );
    ...
    {loading?(<Loading/>):(
      ...
      <div className="package-section text-center">
        <div className="package-section-title">
          <h2>Feutured Packages</h2>
        </div>
        <div className="package-section-subtitle">
      ...
      <div className="package-container container-fluid row mt-3">
        {feturePack.map((p) => (
          <div
            key={p.packId}
            className="package-container-card col-lg-3 col-md-6 mt-3"
          >
